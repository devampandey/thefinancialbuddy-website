...
export function getAllPosts() {
  ...
  const posts = files.map((filename) => {
    ...
    return {
      ...
      // Optional path to a downloadable PDF version of this post (used by
      // Market Pulse issues, which are added by hand after review — not
      // every post has one, so this is "" unless set in frontmatter).
      pdf: data.pdf || "",
      // Punchy, Inshorts-style condensed summary (~60 words) used by the
      // FinShorts (/shorts) swipe feed. Only present on articles drafted
      // after that feature shipped — older posts simply won't appear in
      // the Shorts feed, by design (no backfill).
...
// Posts that have a shortSummary, newest first — the exact set eligible for
// the FinShorts (/shorts) swipe feed.
export function getShortsPosts() {
  return getAllPosts().filter((p) => p.shortSummary);
}
...
export function getPostBySlug(slug) {
  ...
  return {
    ...
    pdf: data.pdf || "",
    shortSummary: data.shortSummary || "",
    // Wraps every rendered markdown table in a scrollable div so the
    // dedicated table CSS (see globals.css) can apply borders/shading, and
    // so a wide table never forces the page wider on mobile.
    html: marked.parse(content).replace(/<table>/g, '<div class="table-wrap"><table>').replace(/<\/table>/g, "</table></div>"),
  };
}
