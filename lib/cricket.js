...
export async function getLiveMatch() {
  try {
    ...
    // otherwise.
    let currentPlay = state === "in" ? await getCurrentPlay(event.id) : null;

    // ESPN's striker/non-striker/bowler tags can linger for a moment with
    // everything reset to zero during a session change (stumps, lunch,
    // etc.) before clearing entirely — a snapshot we've actually seen live.
    // Two batters and a bowler all reading exactly 0 while the batting
    // team's score is already on the board isn't a real cricket state (a
    // genuine new arrival wouldn't coincide with a fresh bowler AND a fresh
    // partner all at once), so treat that combination as stale and drop it
    // rather than show a confusing "everyone's on zero" line.
    if (currentPlay) {
      const battingTeam = teams.find((t) => t.innings?.some((i) => i.isBatting)) || teams[0];
      const currentRuns = parseInt(battingTeam?.score?.split("/")[0], 10) || 0;
      const battersAllZero = currentPlay.batters.every((b) => b.runs === 0 && b.balls === 0);
      const bowlerZero = !currentPlay.bowler || (currentPlay.bowler.overs === 0 && currentPlay.bowler.conceded === 0 && currentPlay.bowler.wickets === 0);
      if (currentRuns > 0 && battersAllZero && bowlerZero) {
        currentPlay = null;
      }
    }
