...
export function generateStaticParams() {
  return getPostsByCategory("Lifestyle").map((post) => ({ slug: post.slug }));
}
...
export default function LifestyleArticlePage({ params }) {
  const post = getPostBySlug(params.slug);
  if (!post || post.category !== "Lifestyle") notFound();
