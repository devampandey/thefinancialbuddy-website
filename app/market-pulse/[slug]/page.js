...
export function generateStaticParams() {
  return getPostsByCategory("MarketPulse").map((post) => ({ slug: post.slug }));
}
...
export default function MarketPulseArticlePage({ params }) {
  const post = getPostBySlug(params.slug);
  if (!post || post.category !== "MarketPulse") notFound();
