      ...
      <div className="mx-auto flex max-w-6xl gap-2 overflow-x-auto px-6 py-2.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {PILLS.map((pill) => (
          <Link
            key={pill.href}
            href={pill.href}
            className="shrink-0 rounded-full border border-gray-300 px-3.5 py-1 text-xs font-medium text-gray-600 transition-colors hover:border-brand hover:text-brand dark:border-gray-700 dark:text-gray-300"
          >
            {pill.label}
          </Link>
        ))}
